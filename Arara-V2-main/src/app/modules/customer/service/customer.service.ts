...
export class CustomerService {
  ...
  removeFromCart(productId: number): Observable<any> {
    const userId = UserStorageService.getUserId(); // Same as getCartByUserId
    return this.http.delete(`${BASIC_URL}api/customer/cart/remove/${userId}/${productId}`, {
      headers: this.createAuthorizationHeader(),  // Add this line
    }).pipe(
      tap(() => this.log('Product removed from cart')),
      catchError(this.handleError<any>('Error removing product from cart'))
    );
  }
  ...
  removeFromWishlist(wishlistId: number): Observable<any> {
    return this.http.delete(`${BASIC_URL}api/customer/wishlist/remove/${wishlistId}`, {
      headers: this.createAuthorizationHeader(),
    });
  }
